			...
			for(var i = 0; i < buildingsArray.length; i++) {
				...
				if(foundBuildings.indexOf(buildingsArray[i]) < 0) {
					...
					newBuilding.properties = {
						name: buildingsArray[i].properties.name,
						type: buildingsArray[i].properties.type
					}
						
					newBuilding.geometry = {
						type: buildingsArray[i].geometry.type,
						coordinates: buildingsArray[i].geometry.coordinates
					}*/
...
db.once('open', function callback () {
	...
	//Create a GeoJSON-compliant mongoose schema for the buildings 
